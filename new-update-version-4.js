...
if (isChrome() || isWindows() || isAndroid()) {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function rebuildVideoDisplay() {
      ...
      // Re-add heading click events after rebuild
    ...
    // Add click event to headings - using direct event listeners
    ...
    function handleHeadingClick(event) {
      const targetTag = event.target.textContent.trim();
      ...
      console.log('Target video index:', targetVideoIndex);
    ...
    // Call this initially and whenever videos are rebuilt
    addHeadingClickEvents();
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function rebuildSafariVideoDisplay() {
      ...
      // Re-add heading click events after rebuild
      addSafariHeadingClickEvents();

      // Initialize first Safari video
      initializeSafariFirstVideo();
    }

    // Add click events to headings for Safari
    function addSafariHeadingClickEvents() {
      const currentHeadings = document.querySelectorAll('.youtube-heading');
      currentHeadings.forEach((heading) => {
        // Remove any existing click listener to avoid duplicates
        heading.removeEventListener('click', handleSafariHeadingClick);
        // Add the click listener
        heading.addEventListener('click', handleSafariHeadingClick);
      });
    }

    function handleSafariHeadingClick(event) {
      const targetTag = event.target.textContent.trim();
      console.log('Safari heading clicked:', targetTag);
      console.log(
        'Safari available videos:',
        videos.map((v) => v.dataset.tag)
      );
      ...
      console.log('Safari target video index:', targetIndex);
    ...
    // Add click events to headings using event delegation
    document.addEventListener('click', function (event) {
      if (event.target.classList.contains('youtube-heading')) {
        const targetTag = event.target.textContent.trim();
        const targetIndex = videos.findIndex(
          (video) => video.dataset.tag === targetTag
        );
