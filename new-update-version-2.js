...
if (isChrome() || isWindows() || isAndroid()) {
    ...
    function setupVideos() {
      ...
      headingTags.forEach((tag, tagIndex) => {
        if (currentOriginalVideos[tagIndex]) {
          ...
          videosPerTag[tag] = 1; // Always 1 video per heading
        }
      });

      return { videos, videosPerTag, headingTags };
    }

    // Initial setup
    let setupResult = setupVideos();
    let videos = setupResult.videos;
    let videosPerTag = setupResult.videosPerTag;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    videos = safariSetupResult.videos;
    let videosPerTag = safariSetupResult.videosPerTag;
